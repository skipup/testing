                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">All PDf</div>
                        </div>
                        <div class="card-body">
                            <a href="">
                                <a href="{{route('admin.pdf.create')}}" class="btn btn-oval btn-success">Create PDF </a>
                            </a>
                        </div>
                                ...
                                <table class="table table-striped my-4 w-100 text text-center" id="">
                                    ...
                                    @foreach($brands as $c)
                                        <tr>
                                            ...
                                            <td>
                                                <img src="{{asset('public'.$c->image)}}" style="width:40px" alt="">
                                            </td>
                                            <td>{{$c->my_title}}</td>
                                            <td>{{$c->en_title}}</td>
                                            <td class="text-center">
                                                @if(auth()->user()->role =='full')
                                                <a href="{{route('admin.pdf.edit',$c->id)}}" class="btn btn-pill-left btn-info d-inline">Edit</a>
                                                <a href="{{route('admin.pdf.show',$c->id)}}" class="btn btn-success btn-info d-inline">View</a>
                                                <form action="{{route('admin.pdf.destroy',$c->id)}}" class="d-inline" method="post">
                                                    {{csrf_field()}}
                                                    {{method_field('Delete')}}
